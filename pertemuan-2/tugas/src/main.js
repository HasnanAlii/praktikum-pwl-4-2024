...
btnTambah.onclick = (e) => {
   ...
   task.id = `${Date.now()}`;
   task.className = `flex justify-center items-center text-black font-bold m-7 gap-2`;
   task.innerHTML = `
         <div class="w-52">
         <p class=" ">${title}</p>
         </div>
         <input type="button" id="btn-selesai" value="SELESAI" data-task="${task.id}"
                class="bg-green-200  px-4 py-2 rounded-lg cursor-pointer hover:bg-slate-500">
         <input type="button" id="btn-hapus" value="HAPUS" data-task="${task.id}"
                class="bg-red-200  px-4 py-2 rounded-lg cursor-pointer hover:bg-slate-500">
   `;
